...
ui <- navbarPage(theme = shinytheme("cerulean"),
                 ...
                 fluidRow(conditionalPanel(
                   condition = "input.metodo == 'Método Block Maxima'",
                   h4("Ajustamos los datos con una distribución generalizada de valores extremos:"))),
...
server <- function(input,output,session){
  ...
  output$tablaNA <-  renderDataTable({ 
    temperaturas <- read.csv("temperaturas.csv",sep="\t")
    temperaturas <- mutate(temperaturas, fecha=paste(dia,mes,anio, sep="-"))
    temperaturas <- mutate(temperaturas,fecha=dmy(temperaturas$fecha))
    temperaturas <- subset(temperaturas, select = c(1,2,3,5,6,10,11) ) 
    if (input$nroest != "Todas") {
    tablaNA <- temperaturas %>% filter(is.na(tmin)) %>% group_by(nroEstacion) %>% 
      summarise(porcentajeNA=round(n()*100/4565,1))
    tablaNA[tablaNA$nroEstacion == input$nroest,]  }
    
    else {tablaNA <- temperaturas %>% filter(is.na(tmin)) %>% group_by(nroEstacion) %>% 
      summarise(porcentajeNA=round(n()*100/4565,1)) %>% arrange(porcentajeNA)
    tablaNA
  ...
  output$paramgev <-  renderPrint ({ 
    if (input$metodo== "Método del Umbral") { 
      ...
      fevd(temperaturas$tmin,threshold="GP")  
