...
server <- function(input,output,session){
  output$serie <- renderPlot({
    ...
    temperaturas %>% filter(Estacion %in% c(input$estac)) %>% filter(between(anio,min(input$year),max(input$year))) %>% group_by(anio,mes,Estacion) %>%
      ...
      ggplot(aes(x=fecha,y=tmin, color=Nombre)) + geom_line()+labs(x="Fecha", y="Temperatura mínima mensual")+
  ...
  output$Mapitav <- renderPlotly({
    ...
    count_df <- data.frame(filter(temperaturas, fecha2 == input$fe[1])$departamento, 
                           filter(temperaturas, fecha2 == input$fe[1])$tmin)
